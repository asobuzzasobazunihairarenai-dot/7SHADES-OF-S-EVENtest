...
function runAction(act, p, onSuccess, contextCard = null, isNewReveal = false) {
    ...
    if (act.type === 'apocalypse_hand') {
        const startApocalypseFlow = () => {
            showSelectionModal("色宣言 (アポカリプス)", "宣言する色を2色選んでください", BASE_COLORS, "card-back-pattern", 2, (selCols) => {
                ...
                if (!drawn) { addLog("山札切れのため終了します。"); if (onSuccess) onSuccess({}); return; }
                showCardModal(drawn, () => {
                    hands[p.id].push(drawn); renderHand();
                    if (targetIds.includes(drawn.colorId) || drawn.colorId === 'rainbow') {
                        ...
                        addLog(`不定期。「${drawn.name}」でした。`);
                        showMessageOverlay("予言失敗。\n効果を終了します。", 1500, () => { if (onSuccess) onSuccess({}); });
                    }
                }, "ドロー＆公開", p.name, "アポカリプス発動中...");
            }, false, () => { if(onSuccess) onSuccess({}); }, null, null, p);
        };
        startApocalypseFlow(); return;
    }
    ...
    return; 
        ...
        const maxLocks = Math.max(...lockCounts.filter(l => l.targetable > 0).map(l => l.total));
        const candidates = lockCounts.filter(l => l.total === maxLocks && l.targetable > 0).map(l => ({ id: l.id, name: `${l.name} (${l.total}枚)`, type: "PLAYER_SELECT" }));
        if (candidates.length === 0) { addLog("奪えるロックカードがないため不発でした。"); onSuccess({}); return; }
        showSelectionModal("最多ロック者選択", "ロックカードを奪う相手を選んでください", candidates, "card-back-pattern", 1, (selPl) => {
            ...
            LOCK_ORDER.forEach(col => {
                ...
                if (slot && slot.length > 0) {
                    const top = slot[slot.length - 1];
                    if (top.type !== 'ETERNAL' && top.type !== 'FIRST' && top.colorId !== 'white' && top.colorId !== 'black') victimLocks.push(top);
                }
            });
            showSelectionModal("カード提供", `${victim.name}さん、渡すロックカードを選んでください`, victimLocks, "card-back-pattern", 1, (selCards) => {
                const stolen = selCards[0]; const slot = collections[victim.id][stolen.colorId];
                slot.splice(slot.indexOf(stolen), 1); hands[p.id].push(stolen);
                addLog(`${victim.name}が「${stolen.name}」を${p.name}に渡しました。`);
                showCardModal(stolen, () => { renderHand(); renderStatus(); renderMyLockArea(); onSuccess({}); }, "カード獲得", p.name, "ロックカードを奪いました");
    ...
    // --- にじいろの呪い：到達/手札共通ロジック ---
        ...
        if (emptySlots.length > 0) {
            showSelectionModal("呪いの封印", `${victim.name}さん、封印するスロットを選んでください`, emptySlots, "card-back-pattern", 1, (slotSel) => {
                const targetCol = slotSel[0];
                collections[victim.id][targetCol.id].push(contextCard);
                addLog(`${victim.name}の${targetCol.name}に呪いが封印された！`);
                
                renderStatus();
                // 修正ポイント：stayOnBoard を削除（または false に）。
                // ロックエリアにカードが移動したため、元のマス（盤面）からは消去する必要があります。
